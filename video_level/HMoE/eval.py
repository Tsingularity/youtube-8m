...
def build_graph(reader,
  ...
  ### Newly
  video_id_batch, model_input_raw, labels_batch, coarse_label_batch, num_frames = get_input_evaluation_tensors(  # pylint: disable=g-line-too-long
  ###
      reader,
      eval_data_pattern,
      batch_size=batch_size,
  ...
  with tf.variable_scope("tower"):
    ...
    ### Newly
    # coarse_predictions = result["coarse_predictions"]
    # tf.summary.histogram("model_coarse_activations", coarse_predictions)
    ###
    ...
    if "loss" in result.keys():
      ...
      ### Newly
      # coarse_label_loss = label_loss_fn.calculate_loss(coarse_predictions, coarse_label_batch)
      ###
  ...
  ### Newly
  # tf.add_to_collection("coarse_loss", coarse_label_loss)
  # tf.add_to_collection("coarse_predictions", coarse_predictions)
  # tf.add_to_collection("coarse_labels", tf.cast(coarse_label_batch, tf.float32))
  ###
...
def evaluation_loop(video_id_batch, prediction_batch, label_batch, loss,
                    ...
                    ### Newly
                    # coarse_predictions, coarse_label_batch, coarse_loss):
                    ###
  ...
  with tf.Session() as sess:
    ...
    ### Newly
    # fetches = [video_id_batch, prediction_batch, label_batch, loss, summary_op, coarse_prediction_batch, coarse_label_batch, coarse_loss]
    ###
    ...
    try:
      ...
      while not coord.should_stop():
        ...
        ### Newly
        # _, predictions_val, labels_val, loss_val, summary_val, coarse_predictions_val, coarse_labels_val, coarse_loss_val = sess.run(
        ###
            fetches)
