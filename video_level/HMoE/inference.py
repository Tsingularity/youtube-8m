...
if __name__ == '__main__':
  ...
  flags.DEFINE_string("certain_step", "",
                      "certain step of checkpoint.")
  flags.DEFINE_string(
      "input_data_pattern", "",
...
def get_input_data_tensors(reader, data_pattern, batch_size, num_readers=1):
  ...
  with tf.name_scope("input"):
    ...
    ### Newly
    video_id_batch, video_batch, unused_labels, unused_coarse_labels, num_frames_batch = (
    ###
  ...
  with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess, gfile.Open(out_file_location, "w+") as out_file:
    ...
    try:
      while not coord.should_stop():
          ...
          predictions_val, = sess.run([predictions_tensor], feed_dict={input_tensor: video_batch_val, num_frames_tensor: num_frames_batch_val, phase: False})
          now = time.time()
