...
def build_graph(reader,
  ...
  ### Newly
  unused_video_id, model_input_raw, labels_batch, coarse_labels_batch, num_frames = (
  ###
  ...
  ### Newly
  tower_coarse_labels = tf.split(coarse_labels_batch, num_towers)
  ###
  tower_num_frames = tf.split(num_frames, num_towers)
  tower_gradients = []
  tower_predictions = []
  ### Newly
  tower_coarse_predictions = []
  tower_coarse_losses = []
  ###
  tower_label_losses = []
  tower_reg_losses = []
  
  phase = tf.constant(True, dtype=tf.bool)
    ...
    with tf.device(device_string % i):
      with (tf.variable_scope(("tower"), reuse=True if i > 0 else None)):
        with (slim.arg_scope([slim.model_variable, slim.variable], device="/cpu:0" if num_gpus!=1 else "/gpu:0")):
          ...
          ### Newly
          coarses = result["coarse_predictions"]
          tower_coarse_predictions.append(coarses)
          ###
          ...
          if "loss" in result.keys():
            ...
            ### Newly
            coarse_loss = label_loss_fn.calculate_loss(coarses, tower_coarse_labels[i])
            ###
          ...
          ### Newly
          tower_coarse_losses.append(coarse_loss)
          ###

          # Incorporate the L2 weight penalties etc.
          # final_loss = regularization_penalty * reg_loss + label_loss
          ### Newly 
          final_loss = regularization_penalty * reg_loss + label_loss + coarse_loss
          ###
  ...
  ### Newly
  coarse_loss = tf.reduce_mean(tf.stack(tower_coarse_losses))
  ...
  ### Newly
  tf.add_to_collection("coarse_loss", coarse_loss)
  tf.add_to_collection("coarse_predictions", tf.concat(tower_coarse_predictions, 0))
  ###
    ...
    with sv.managed_session(target, config=self.config) as sess:
      try:
        ...
        while (not sv.should_stop()) and (not self.max_steps_reached):
          ...
          if self.is_master and global_step_val % 128 == 0 and self.train_dir:
