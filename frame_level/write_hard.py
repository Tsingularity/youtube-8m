...
if __name__ == '__main__':
  ...
  flags.DEFINE_bool("clear_device",False,"whether clear device")
...
#                                                  for pair in line) + "\n"
    ...
    print labels.shape, video_index
    raw_label = [numpy.argmax(labels[video_index])*1000]
...
def inference(reader, train_dir, data_pattern, out_file_location, batch_size, top_k):
  ...
  with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess, tf.python_io.TFRecordWriter(out_file_location) as out_file:
    ...
    hard_predictions = tf.get_collection("hard_predictions")[0]
    ...
    try:
      while not coord.should_stop():
          ...
          hard_predictions_val = sess.run([hard_predictions],feed_dict={input_tensor:video_batch_val,num_frames_tensor:num_frames_batch_val})
          ...
          for sample in format_lines(video_id_batch_val, video_batch_val, hard_predictions_val):
